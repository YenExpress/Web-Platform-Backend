...
import (
	DBC "yenexpress/internal/api/database/connectors"
)
...
func (repo *BUserRepository[userSchema]) GetByEmail(email string) (*userSchema, error) {
	...
	err := DBC.DB.Where("email = ?", email).First(&schema).Error
	if err == nil {
		return schema, nil
	}
	return nil, err
...
func (repo *BUserRepository[userSchema]) GetByID(ID interface{}) (*userSchema, error) {
	var schema *userSchema
	err := DBC.DB.Where("ID = ?", ID).First(&schema).Error
	if err == nil {
		return schema, nil
	}
	return nil, err
